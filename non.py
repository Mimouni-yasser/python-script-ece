        ...
        if(input(style.RED+"[?] - do you want to generate the QR codes? [y/n]: " + style.RESET) == 'y'):
            ...
            for id in collection.find({}):
                ...
                if(id['apogee'] == 1924577):
                    work = 1
                if(work == 1):
                    print(style.RED+"[+] - " + style.RESET + "Generating QR code for {} ({}/{})" .format(id['name '], i, collection.count_documents({})) )
                    os.system("python3 \"generate_QR\gen.py\" " + str(id['_id']) + "> NUL")
            ...
            if(choice == 1):
                ...
                try:
                         yag = yagmail.SMTP("yasmimouni2", "msgppgzwxuxoesix")
                          #yag.send(to="anfassi26@gmail.com", subject="test", contents="../email html/index.html")
                except:
                    print(style.RED + "error connecting to email server" + style.RESET)
                    exit()
                for x in collection.find():
                    print(style.YELLOW + style.BOLD + "name: " + style.RESET + x['nom ']+" "+style.YELLOW + style.BOLD)
